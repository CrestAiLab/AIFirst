...
import { communityPostsCountQuery, communityPostsPaginatedQuery } from "@/lib/sanity/queries"
...
export default async function CommunityPage({ searchParams }: Props) {
  ...
  const total = await client
    .fetch(communityPostsCountQuery, {}, { next: { tags: ["community"] } })
    .catch(() => 0)
  ...
  const communityPosts = await client
    .fetch(communityPostsPaginatedQuery, { start: from, end: to }, { next: { tags: ["community"] } })
    .catch(() => [])
        ...
        <div className="container">
          ...
          {communityPosts && communityPosts.length > 0 ? (
            <>
              ...
              <ListPagination page={page} totalPages={totalPages} path="/community" />
            </>
          ) : (
